function [Sc1 Sc2]=MakeScaleMovieE(Direct,Start,Stop,MovieName, FPS,eps, Scale1, Scale2, Cr)
%Cr [L R T B]
load([Direct 'Vars/PreRunVars.mat'], 'ImgStart', 'SetStop');
if nargin == 1, Start=ImgStart; Stop=SetStop(1); 
elseif nargin == 8, Cr=[0 0 0 0]; end
...
% create the AVI file object, set fps, compression, etc.
    ...
    for i=Start:Stop
        ...
        C1=C1(T:B,L:R)/Scale1;
        C2=C2(T:B,L:R)/Scale2;
...
%         Img(:,:,3)=C2.*(C2>eps).*(C2<=1)+double(C2>1);
        ...
        writeVideo(aviobj,f);
        %Display Progress
        MikesPogressBar(i-Start+1,Length,h);
