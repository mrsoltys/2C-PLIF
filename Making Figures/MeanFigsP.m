function [ A1, A2, Xs, Ys ] = MeanFigsP( MeanName , S, Shift, TitleText, FileName,eps,A1,A2)

%Intilize Figure
    Figure1=figure(1);
    clf(Figure1);
set(Figure1,'defaulttextinterpreter','latex','PaperUnits','inches','PaperSize',[11 8.5],'PaperPosition',[.5,.5,10,7.5]);
...
load(['Vars/Eps' sprintf('%.3f', eps) '/RMSEe' MeanName]);
    ...
    Img=ColorChangeWhite(RMSE1,RMSE2,.5);
    imshow(Img,'XData',Xs,'YData',Ys);
    hold on;axis on;
...
subplot(2,2,4);
    ...
    B(1,:)= [0:1/31:1 ones(1,32)];
        ...
        colormap(B');caxis([-1 1]);colorbar('location','EastOutside')
    hold on;axis image;
